...
from school_db.models import Course, Instructor, Student
from school_db.const_data import view_information
...
def problem_one(request):
    students_gpa_above_3 = Student.objects.filter(gpa__gt=3.0).order_by('-gpa')
    for student in students_gpa_above_3:
        print(
...
def problem_two(request):
    instructors_hired_prior_2010 = Instructor.objects.filter(
        hire_date__lt='2010-01-01')
    for instructor in instructors_hired_prior_2010:
        print(
...
def problem_four(request):
    ...
    print(f'Students Count: {students_total}')
    print(f'Courses Count: {courses_total}')
    print(f'Instructors Count: {instructors_total}')
    return complete(request)
...
def problem_five(request):
    new_student = Student.objects.create(
        first_name='Jiang', last_name='Wang', year=2022, gpa=3.5)
...
def problem_six(request):
    ...
    student_id = 11
    Student.objects.get(id=student_id).update(gpa=3.2)
...
def bonus_problem(request):
    teachers_single_course = Instructor.objects.annotate(
        num_of_courses=Count('course')).filter(num_of_courses=1)
    for teacher in teachers_single_course:
      print(f'Instructor Name: {teacher.first_name} {teacher.last_name}')
    return complete(request)
